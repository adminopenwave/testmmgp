...
class CreateRequest extends Component {
  ...
  searchCompanynotry = () => {    
    ...
    .then(data => {
      console.log('=>',data)
      if(data){
        this.setState({ companynolistArray: data.CompanyList.map((option) =>option.CompanyNo) }
        )     
        if(this.state.companynolistArray.length > 500){
         this.setState({optioncompanyno : data.CompanyList.map((option) =>option.CompanyNo)})          
        }  
        else
        {
          this.setState({optioncompanyno : this.state.companynolistArray}) 
        }
      }
    })
  }
